...
class oltController {
    ...
    static ListarRamais = (req, res) => {
        ramaisModel.find((err, user)=>{
        res.status(200).json(user)
        }).sort({_id: -1})
    }

    static ListarVlans = (req, res) => {
      let oltIP = req.body.oltIp;
      let oltPon = req.body.oltPon;
      vlanModel.find({ oltIP: oltIP, oltPon: oltPon }, (err, vlans) => {
        if (err) {
          res.status(500).json({ message: `Back-end err: ${err}` });
        } else {
          if (vlans.length === 0) {
            res.status(200).json({ message: "No VLANs found" });
          } else {
